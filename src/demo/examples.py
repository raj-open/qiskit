...
def action_prepare_circuit_and_job(
    ...
    def action(
        ...
        # create circuit:
        display(
            HTML(f"<h3>Example quantum circuit with {'backend' if queue else 'simulator'}</h3>")
        )
        ...
        if N > 0:
            display(
                QkVisualisation.plot_distribution(
                    counts, title=f"Measurements (batch size: {N})"
                )
            )
            display(
                QkVisualisation.plot_distribution(
                    counts_0, title=f"Measurements of QBit 0 (batch size: {N})"
                )
            )
            display(
                QkVisualisation.plot_distribution(
                    counts_1, title=f"Measurements of QBit 1 (batch size: {N})"
                )
            )
        else:
            display(
