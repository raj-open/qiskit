...
def action_prepare_circuit_and_job(
    ...
    - `verbose` - <boolean> if `true` will print out information about Grover circuit construction.
    ...
    def action(
        ...
        circuit = grover_algorithm_from_sat(problem=problem, prob=prob, verbose=verbose)
    ...
    action(num_shots=num_shots, verbose=verbose, problem=problem, prob=prob)
    return
...
def action_display_statistics(
    ...
    def action(job: IBMJob):
        ...
        if N > 0:
            fig, ax = mplt.subplots(2, 1, figsize=(20, 20), dpi=360)
            mplt.rcParams.update(
                {
                    "figure.facecolor": (243 / 255, 243 / 255, 243 / 255, 0.8),
            ...
            ax[0].set_title(f"Measurements (batch size: {N})")
            ax[1].set_title(f"Measurements of search bits (batch size: {N})")
            QkVisualisation.plot_distribution(counts, ax=ax[0])
            QkVisualisation.plot_distribution(counts_inputs, ax=ax[1])
        else:
            display(
...
def basic_action_prepare_problem(
    ...
    if verbose:
        if path is not None:
            ...
            display(HTML("Load SAT Problem from text entry</b>."))
