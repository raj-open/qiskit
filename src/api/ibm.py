...
class CreateBackend(QkBackend):
    ...
    def __init__(
        self, option: BACKEND | BACKEND_SIMULATOR, n: int = 1, force_reload: bool = False
    ):
        self.option = option
        self.nr_qubits = n
        self.provider = None
...
def connect_to_backend(
    option: BACKEND | BACKEND_SIMULATOR, n: int = 1, force_reload: bool = False
) -> Callable[
...
def display_backends():
    ...
    names_queue = [
        str(be)
        for be in backends
        if isinstance(be, IBMQBackend) and not isinstance(be, IBMQSimulator)
    ]
