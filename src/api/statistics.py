    ...
    *bits: list[int],
    ...
    if isinstance(counts_raw, list):
        if len(counts_raw) > 0:
            ...
            counts = {key: sum([count.get(key, 0) for count in counts_raw]) for key in keys}
    else:
        counts = counts_raw
    # NOTE: Qiskit places spaces in key to signify different blocks of registered cbits. Remove these here:
    counts = {
        re.sub(pattern=r"\s+", repl="", string=key): value for key, value in counts.items()
    }
    ...
    statistic = [
        {
            key: sum(
                [value for key_long, value in counts.items() if match_subkey(key_long, key, C)]
            )
...
def match_subkey(key_long: str, key: str, C: list[int]) -> bool:
    return all(key_long[index] == a for index, a in zip(C, key))
