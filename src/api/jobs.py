...
def recover_job(
    ...
    if as_widget:
        ...
        job = (
            retrieve_job(queue=queue, job_id=job_id, backend_option=backend_option) or last_job
        )
        # create widget:
        ...
        def dec(action: Callable[Concatenate[IBMQJob, ARGS], T]) -> Callable[ARGS, None]:
            ...
            def wrapped_action(**kwargs) -> None:
                # retrieve job or else use latest job:
                job = (
                    retrieve_job(queue=queue, job_id=job_id, backend_option=backend_option)
                    or last_job
                )
    ...
    def create(self):
        ...
        if self.queue:
            ...
            options_backend = [("—", None)] + [
                (e.value, e) for e in enums if e != BACKEND.LEAST_BUSY
            ]
            options_jobs = [("—", None)] + (
                [] if self.job is None else [(self.job.job_id(), self.job)]
            )
            try:
