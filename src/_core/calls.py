...
def keep_calm_and_carry_on(
    *actions: Callable[[], Result[CallState, CallState]],
    ...
    """
    return Result.collect((call_action_passively(action) for action in actions)).and_then(
        post_process_results
    )


def call_action_passively(
    action: Callable[[], Result[CallState, CallState]],
) -> Result[CallState, Nothing]:  # pragma: no cover
    """
...
def post_process_results(
    states: tuple[CallState, ...],
) -> Result[CallState, CallState]:  # pragma: no cover
    """
...
def run_safely(tag: str | None = None, error_message: str | None = None):
    ...
    def dec(
        action: Callable[ARGS, Result[V, CallState]],
    ) -> Callable[ARGS, Result[V, CallState]]:
        """
        ...
        def wrapped_action(*_, **__) -> Result[V, CallState]:
            ...
            return (
                ...
                .flatmap(lambda value: value)
            )  # necessary to flatten result.
...
def to_async(executor: Any | None = None):
    ...
    def dec(
        routine: Callable[ARGS, T],
    ) -> Callable[Concatenate[AbstractEventLoop, ARGS], Awaitable[T]]:
        """
